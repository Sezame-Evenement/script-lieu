...
const sortedHours = Array.from(currentlySelectedHours).sort((a, b) => a - b);
const earliestHour = sortedHours[0];
const latestHour = sortedHours[sortedHours.length - 1];

if (earliestHour != null && !currentlySelectedHours.has(earliestHour - 1)) {
addTimeRange(earliestHour - 1, formattedSelectedDate, dataToUpdate, selectedDate);
}
if (latestHour != null && !currentlySelectedHours.has(latestHour + 1)) {
addTimeRange(latestHour + 1, formattedSelectedDate, dataToUpdate, selectedDate);
}

previouslySelectedHours.forEach(hour => {
...
console.log("Adding time range:", range);
...
let adjustForPrevDay = hour < 0;
let adjustForNextDay = hour > 23;

if (adjustForPrevDay) {
hour = 23;
targetDate.setDate(targetDate.getDate() - 1);
} else if (adjustForNextDay) {
hour = 0;
targetDate.setDate(targetDate.getDate() + 1);
}
...
if (index !== -1) {
console.log("Removing time range:", range);
data[targetFormattedDate].splice(index, 1);
}
}

if (adjustForPrevDay || adjustForNextDay) {
const adjacentDate = new Date(targetDate);
adjustForPrevDay ? adjacentDate.setDate(adjacentDate.getDate() + 1) : adjacentDate.setDate(adjacentDate.getDate() - 1);
const adjacentFormattedDate = adjacentDate.toLocaleDateString('fr-CA');
const adjacentRange = adjustForPrevDay ? '0h à 1h' : '23h à 0h';

if (data[adjacentFormattedDate] && data[adjacentFormattedDate].includes(adjacentRange)) {
const adjacentIndex = data[adjacentFormattedDate].indexOf(adjacentRange);
console.log("Also removing adjacent time range:", adjacentRange);
data[adjacentFormattedDate].splice(adjacentIndex, 1);
}
}
}
...
const openingHour = parseInt('{{wf {&quot;path&quot;:&quot;horaire-d-ouverture-24h&quot;,&quot;type&quot;:&quot;PlainText&quot;\} }}'.split(":")[0]);
